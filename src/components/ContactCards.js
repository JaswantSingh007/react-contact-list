            ...
            {contactList?.map((contact, index)=> (
                <div key={index} className="bg-white h-auto w-auto rounded-lg shadow-md p-7 ">
                    ...
                    <div className='text-center mt-4'>
                        <button onClick={()=>{handelUpdate(index)}} type="button" className='border rounded-full pt-1 pb-1 pl-2 pr-2 shadow-md mr-3 mt-3 bg-yellow-300'>Update</button>
                        <button onClick={()=>{handelDelete(index)}} type="button" className='border rounded-full pt-1 pb-1 pl-2 pr-2 shadow-md mt-3 bg-red-600'>Delete</button>
                    </div>
                </div>
            ))
            }
